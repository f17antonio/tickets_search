...
fn get_all_tickets(con: &Connection, departure_code: String, arrival_code: String, departure_time: i64) -> Vec<Vec<Ticket>>{
    ...
    if direct_tickets.len() > 0 {
        for (dirrect_arrival_code, departure_time) in direct_tickets {
            if dirrect_arrival_code == arrival_code {
                tickets.push(vec![get_ticket_by_time(con,&dirrect_arrival_code, &departure_time)]);
            } else {
                let transfer_tickets = get_transfer_tickets(con, &arrival_code, dirrect_arrival_code, departure_time);
...
fn get_transfer_tickets(con: &Connection, arrival_code: &String, dirrect_arrival_code: String, departure_time: i32) -> Vec<Vec<Ticket>> {
    let mut transfer_tickets: Vec<Vec<Ticket>> = Vec::new();
    let direct_ticket = get_ticket_by_time(con,&dirrect_arrival_code, &departure_time);
    let start_time = direct_ticket.arrival_time as i64 + 10800;
    let end_time = direct_ticket.arrival_time as i64 + 82800;
    let tickets_with_transfer = get_tickets_by_time(con, dirrect_arrival_code, start_time, end_time);
    if tickets_with_transfer.len() > 0 {
        for (transfer_arrival_code, transfer_departure_time) in tickets_with_transfer {
            if &transfer_arrival_code == arrival_code {
                transfer_tickets.push(vec![
                    direct_ticket.clone(),
                    get_ticket_by_time(con,&transfer_arrival_code, &transfer_departure_time)
                ]);
            }
        }
    }
